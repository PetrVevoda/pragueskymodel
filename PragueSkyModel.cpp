...
std::string getStringCmdOption(char** begin, char** end, const std::string& option, const std::string& defaultValue) {
...
constexpr std::array<double, 11> wavelengths = { 340.0, 380.0, 420.0, 460.0, 500.0, 540.0, 580.0, 620.0, 660.0, 700.0, 740.0 };
...
int main(int argc, char* argv[]) {    
	if (cmdOptionExists(argv, argv + argc, "-h") || cmdOptionExists(argv, argv + argc, "--help")) {
		...
		std::cout << "   -mod ... what quantity to output, use 0 for sky radiance, 1 for sun radiance, 2 for polarisation and 3 for transmittance, default 0\n";
		std::cout << "   -out ... output file, default \"test.exr\"\n";
		std::cout << "   -res ... square output image resolution, default 512 pixels\n";
		std::cout << "   -vis ... ground level visibility, valid range [20, 131.8] kilometers, default 59.4 kilometers\n";
		return 1;
	}	
    ...
    ArPragueSkyModelState* state = arpragueskymodelstate_alloc_init("PragueSkyModelDataset.dat");
    ...
    for (int x = 0; x < resolution; x++)
    {
        for (int y = 0; y < resolution; y++)
        {
            ...
            for (int wl = 0; wl < 11; wl++)
            {
				...
				{
				case 1:
					spectrum[wl] = arpragueskymodel_solar_radiance(
						state,
						theta,
						gamma,
						shadow,
					...
					break;
				case 2:
					spectrum[wl] = std::abs(arpragueskymodel_polarisation(
						state,
						theta,
						gamma,
						solarElevationAtViewpoint,
					...
					break;
				case 3:
					spectrum[wl] = arpragueskymodel_tau(
						state,
						theta,
						altitudeOfViewpoint,
						visibility,
						wavelengths[wl],
						5.78960446186580977117855E+76
					);
					break;
				default:					
					spectrum[wl] = arpragueskymodel_radiance(
						state,
						theta,
						gamma,
						shadow,
    ...
    arpragueskymodelstate_free(state);
